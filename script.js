...
const lines=["Initializing system...","Loading modules...","Connecting to server...",];let index=0;function displayLine(){if(index<lines.length){appendOutput(lines[index]);index++;setTimeout(displayLine,1000)}else{appendOutput(welcomeArt);appendOutput(helpText);userInput.focus()}}
document.addEventListener('DOMContentLoaded',()=>{displayLine();userInput.focus()});const commands={"/help":"Available commands:\n/aboutme - Learn about me\n/contact - Get in touch\n/education - Display Academic Portfolio\n/projects - View my current project portfolio\n/clear - Clear the screen","/aboutme":"Name: Justin Derenthal\nField: Computer Scientist\nInterests: AI, Web Development, Cybersecurity","/contact":contactText,"/education":`
    ...
    `,"/clear":""};
const availableCommands=["/help","/aboutme","/contact","/education","/projects","/clear"];let history=[],historyIndex=-1;userInput.addEventListener("input",function(){if(!userInput.value.startsWith("/")){userInput.value="/"+userInput.value}});userInput.addEventListener("keydown",function(event){if(event.key==="Enter"){const command=userInput.value.trim();if(command){history.push(command);historyIndex=history.length;processCommand(command)}userInput.value="";userInput.focus()}else if(event.key==="ArrowUp"){if(historyIndex>0){historyIndex--;userInput.value=history[historyIndex]}}else if(event.key==="ArrowDown"){if(historyIndex<history.length-1){historyIndex++;userInput.value=history[historyIndex]}else{userInput.value=""}}else if(event.key==="Tab"){event.preventDefault();const currentValue=userInput.value.trim();const match=availableCommands.find(cmd=>cmd.startsWith(currentValue));if(match)userInput.value=match}});
